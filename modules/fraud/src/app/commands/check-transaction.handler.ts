...
export class CheckTransactionHandler implements ICommandHandler<CheckTransactionCommand> {
  constructor(
    private readonly publisher: EventPublisher,
  ) { }

  async execute(command: CheckTransactionCommand): Promise<void> {
    const IncomingTransaction = this.publisher.mergeClassContext(IncomingTransactionAggregate);

    const incomingTransaction = new IncomingTransaction(command.transactionPayload);
